...
import PS_drawingsurface # only used to inherit for: KfFileExportSurface
...
class SingleOverlay(object):
    ''' appears to be for converions of value to bytes
        - incomplete
    '''
    def __init__(self):
        pass
...
class KfFileExportSurface(PS_drawingsurface.KfDrawingSurface):
    def __init__(self):
        self.fileName = ""
        self.scale = 0.0
...
class KfBinaryFileExportSurface(KfFileExportSurface):
    ...
    def startFile(self):
        self.stream = TFileStream() #delphi_compatability.TFileStream().Create(self.fileName, delphi_compatability.fmCreate or UNRESOLVED.fmShareExclusive)
...
class KfDrawingSurfaceFor3DS(KfBinaryFileExportSurface):
    ...
    def popChunkStartAndFixupChunkSize(self):
        ...
        self.stream.Seek(startSize + 2, 2)#!!delphi_compatability.soFromBeginning)
        self.writeDword(chunkSize)
        self.stream.Seek(0, 2)#!!delphi_compatability.soFromEnd)
...
class KfDrawingSurfaceForLWO(KfBinaryFileExportSurface):
    ...
    def popChunkStartAndFixupChunkSize(self):
        ...
        chunkSize = totalSize - startSize
        # skip back four
        self.stream.Seek(startSize - 4, 2)#!!delphi_compatability.soFromBeginning)
