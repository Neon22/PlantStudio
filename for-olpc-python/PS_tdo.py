...
""" Appears to export and import geometry
"""
...
class KfObject3D(object):
    ...
    def isSameAs(self, other):
        ...
        for i in range(0, len(self.points)):
            ...
            if (int(thePoint.x) != int(otherPoint.x)) or (int(thePoint.y) != int(otherPoint.y)) or (int(thePoint.z) != int(otherPoint.z)):
    ...
    def addPointIfNoMatch(self, aPoint, matchDistance):
        for i in range(0, len(self.points)):
            if PS_3dsupport.KfPoint3D_matchXYZ(aPoint, self.points[i], matchDistance):
    ...
    def adjustForOrigin(self):
        ...
        for i in range(0, len(self.points)):
            if i != self.originPointIndex:
                #PS_3dsupport.KfPoint3D_subtract(self.points[i], self.points[self.originPointIndex])
                self.points[i].subtract(self.points[self.originPointIndex])
        #makes the first point zero
        #PS_3dsupport.KfPoint3D_setXYZ(self.points[self.originPointIndex], 0.0, 0.0, 0.0)
        self.points[self.originPointIndex].setXYZ(0.0, 0.0, 0.0)
    ...
    def bestPointForDrawingAtScale(self, turtleProxy, origin, bitmapSize, scale):
        ...
        turtle.reset()
        for point in self.points:
            PS_3dsupport.KfPoint3D_addPointToBoundsRect(self.boundsRect, turtle.transformAndRecord(point, scale))
            # PDF FIX -- no TPOINT?
        result = SinglePoint()
    ...
    def addPointString(self, stream):
        aPoint3D = PS_3dsupport.KfPoint3D()
        x = stream.nextInteger()
        y = stream.nextInteger()
        z = stream.nextInteger()
        #PS_3dsupport.KfPoint3D_setXYZ(aPoint3D, x, y, z)
        aPoint3D.setXYZ(x, y, z)
    ...
    def addTriangleString(self, stream):
        ...
        if (p1 == 0) or (p2 == 0) or (p3 == 0) or (p1 > lenPoints) or (p2 > lenPoints) or (p3 > lenPoints):
            print "whoops"
            #!!MessageDialog("Bad triangle: %d %d %d.  Point indexes must be between 1 and %d." % (p1, p2, p3, lenPoints), delphi_compatability.TMsgDlgType.mtError, [mbOK, ], 0)
        else:
            self.addTriangle(KfIndexTriangle().createABC(p1, p2, p3))

    def writeToFile(self, fileName):
        outputFile = TextFile()
        outputFile.assignFilename(fileName)
        try:
            # v1.5
            usupport.setDecimalSeparator()
            outputFile.open_for_writing()
    ...
    def writeToMemo(self, aMemo):
        ...
        for point in self.points:
            aMemo.Lines.Add("  Point=%d %d %d" % (int(point.x), int(point.y), iter(point.z)))
        aMemo.Lines.Add("; Origin= %d" % (self.originPointIndex))
        for triangle in self.triangles:
            aMemo.Lines.Add("  Triangle=%d %d %d" % (triangle.pointIndexes[0], triangle.pointIndexes[1], triangle.pointIndexes[2]))
    ...
    def writeToFileStream(self, outputFile, embeddedInPlant):
        ...
        for point in self.points:
            writeln(outputFile, "  Point=%d %d %d" % (int(point.x), int(point.y), int(point.z)))
        writeln(outputFile, "; Origin=%d" % (self.originPointIndex))
        for triangle in self.triangles:
            writeln(outputFile, "  Triangle=%d %d %d" % (triangle.pointIndexes[0], triangle.pointIndexes[1], triangle.pointIndexes[2]))
        if embeddedInPlant:
    ...
    def readFromFileStream(self, inputFile, embeddedInPlant):
        ...
        if embeddedInPlant:
            ...
            if embeddedInPlant and inputLine.strip() == "":
                ...
                inputLine = inputFile.readln()
            if inputLine and (inputLine[0] == ";") and not string_in("ORIGIN",inputLine):
        ...
        while inputLine != None:
            if embeddedInPlant and inputLine.strip() == "":
                ...
                pass
            elif inputLine and (inputLine[0] == ";") and (not string_in("ORIGIN", inputLine)):
                # cfk change for v1.6b2 mistake with origin, placed with comment
                pass
            elif (not embeddedInPlant) and (string_in("[", inputLine)):
                ...
                elif string_in("ORIGIN", fieldType):
    ...
    def readFromDXFFile(self, inputFile):
        ...
        identifierLine = inputFile.readln()
        while identifierLine != None:
            identifierLine = identifierLine.strip()
            valueLine = inputFile.readln()
            if valueLine == None:
            ...
            if identifierLine == "10":
                ...
                # first point
                newPoints[0].x = float(valueLine)
            if identifierLine == "20":
                newPoints[0].y = -1 * float(valueLine)
            if identifierLine == "30":
                newPoints[0].z = float(valueLine)
            if identifierLine == "11":
                # second point
                newPoints[1].x = float(valueLine)
            if identifierLine == "21":
                newPoints[1].y = -1 * float(valueLine)
            if identifierLine == "31":
                newPoints[1].z = float(valueLine)
            if identifierLine == "12":
                # third point
                newPoints[2].x = float(valueLine)
            if identifierLine == "22":
                newPoints[2].y = -1 * float(valueLine)
            if identifierLine == "32":
                newPoints[2].z = float(valueLine)
        ...
        try:
            ...
            while part != "":
                ...
                if firstLetter.upper() == "N":
                    self.setName(part[2:-1]) # betweenBrackets(part)
                elif firstLetter.upper() == "P":
                    partStream.onStringSeparator(part[2:-1], " ")
                    self.addPointString(partStream)
                elif firstLetter.upper() == "T":
                    partStream.onStringSeparator(part[2:-1], " ")
        ...
        try:
            ...
            if rotateCount > 1:
                ...
                for i in range(1, rotateCount):
                    writeln(outputFile, chr(9) + "object { " + nameString + " rotate %d*365/%d*y }" % (i, rotateCount))
                writeln(outputFile, "}")
        finally:
            outputFile.close()
    ...
    def writeTriangleToPOV_INCFIle(self, outputFile, p1, p2, p3):
        ...
        write(outputFile, "%d. %d, %d>, >" % (int(p1.x), -int(p1.y), int(p1.z)))
        write(outputFile, "%d. %d, %d>, >" % (int(p2.x), -int(p2.y), int(p2.z))) #IntToStr(int(p2.x)) + ", " + IntToStr(-int(p2.y)) + ", " + IntToStr(int(p2.z)) + ">, <")
        writeln(outputFile, "%d, %d, %d> }" % (int(p3.x), -int(p3.y), int(p3.z)))
