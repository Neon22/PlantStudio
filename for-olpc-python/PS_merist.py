...
class PdMeristem(PS_part.PdPlantPart):
    ...
    def nextDay(self):
        ...
        try:
            ...
            raise
            PS_support.messageForExceptionType(e, "PdMeristem.nextDay")
    ...
    def contemplateBranching(self):
        ...
        if self.phytomerAttachedTo.distanceFromApicalMeristem() < self.plant.pMeristem.branchingDistance:
            if self.plant.pMeristem.branchingDistance == 0:
                ...
                decisionPercent = self.plant.pMeristem.branchingIndex * min(1.0, max(0.0, PS_math.safedivExcept(self.phytomerAttachedTo.distanceFromApicalMeristem(), self.plant.pMeristem.branchingDistance, 0)))
    ...
    def accumulateOrCreatePhytomer(self):
        ...
        try:
            ...
            if shouldCreatePhytomer:
                fractionOfOptimalSize = PS_math.safedivExcept(self.liveBiomass_pctMPB, optimalInitialBiomass_pctMPB, 0)
            ...
            #PDF PORT ADDED RAISE FOR TESTING
            raise
            PS_support.messageForExceptionType(e, "PdMeristem.accumulateOrCreatePhytomer")
    ...
    def accumulateOrCreateInflorescence(self):
        ...
        try:
            if self.phytomerAttachedTo.isFirstPhytomer or not self.isReproductive or not self.isActive:
                return
            shouldCreateInflorescence = False #!!
            optimalInitialBiomass_pctMPB = PS_inflor.PdInflorescence.optimalInitialBiomass_pctMPB(self.plant, self.gender)
            if (self.liveBiomass_pctMPB >= optimalInitialBiomass_pctMPB):
            ...
            raise
            PS_support.messageForExceptionType(e, "PdMeristem.accumulateOrCreateInflorescence")
        ...
        try:
            if mode == kActivityNone:
                ...
                try:
                    ...
                    PS_support.messageForExceptionType(e, "PdMeristem.traverseActivity (vegetative demand)")
                ...
                if (self.phytomerAttachedTo.isFirstPhytomer) and (not self.isApical):
                    return
                try:
                    self.biomassDemand_pctMPB = PS_travers.linearGrowthResult(self.liveBiomass_pctMPB, self.plant.pInflor[self.gender].optimalBiomass_pctMPB, self.plant.pInflor[self.gender].minDaysToCreateInflorescence)
                    traverser.total = traverser.total + self.biomassDemand_pctMPB
                except Exception, e:
                    PS_support.messageForExceptionType(e, "PdMeristem.traverseActivity (reproductive demand)")
                ...
                try:
                    ...
                    PS_support.messageForExceptionType(e, "PdMeristem.traverseActivity (vegetative growth)")
                ...
                try:
                    ...
                    PS_support.messageForExceptionType(e, "PdMeristem.traverseActivity (reproductive growth)")
                ...
                if not self.isActive:
                    ...
                    return
                if (mode == kActivityVegetativeBiomassThatCanBeRemoved) and (self.isReproductive):
                    return
                if (mode == kActivityReproductiveBiomassThatCanBeRemoved) and (not self.isReproductive):
                    return
                try:
                    # a meristem can lose all of its biomass
                    traverser.total = traverser.total + self.liveBiomass_pctMPB
                except Exception, e:
                    PS_support.messageForExceptionType(e, "PdMeristem.traverseActivity (removal request)")
                ...
                if not self.isActive:
                    return
                if (mode == kActivityRemoveVegetativeBiomass) and (self.isReproductive):
                    return
                if (mode == kActivityRemoveReproductiveBiomass) and (not self.isReproductive):
                    return
                try:
                    ...
                    PS_support.messageForExceptionType(e, "PdMeristem.traverseActivity (removal)")
            ...
            raise
            PS_support.messageForExceptionType(e, "PdMeristem.traverseActivity")
    ...
    def countPointsAndTrianglesFor3DExportAndAddToTraverserTotals(self, traverser):
        ...
        if self.plant.pAxillaryBud.tdoParams.scaleAtFullSize > 0:
            ...
            self.addExportMaterial(traverser, kExportPartMeristem, -1)

    def report(self):
        PS_part.PdPlantPart.report(self)
        #udebug.DebugPrint('meristem, age %d biomass %f' % ( PS_part.PdPlantPart.age,  PS_part.PdPlantPart.liveBiomass_pctMPB))
    ...
    def createFirstPhytomer(self):
        ...
        try:
            ...
            raise
            PS_support.messageForExceptionType(e, "PdMeristem.createFirstPhytomer")
        return result
    ...
    def createInflorescence(self, fractionOfOptimalSize):
        if self.plant.partsCreated > udomain.domain.options.maxPartsPerPlant_thousands * 1000:
            # v1.6b1
            return
        newInflorescence = PS_inflor.PdInflorescence()
    ...
    def createPhytomer(self, fractionOfFullSize):
        ...
        if (self.phytomerAttachedTo != None):
            ...
            if self.plant.pMeristem.branchingAndLeafArrangement == kArrangementAlternate:
                leftMeristem = self.createAxillaryMeristem(kDirectionLeft)
                if self.plant.pMeristem.branchingIsSympodial:
                    leftMeristem.setIfActive(True)
            else:
                leftMeristem = self.createAxillaryMeristem(kDirectionLeft)
                rightMeristem = self.createAxillaryMeristem(kDirectionRight)
                if self.plant.pMeristem.branchingIsSympodial:
    ...
    def draw(self):
        ...
        daysToFullSize = 5
        scale = (self.plant.pAxillaryBud.tdoParams.scaleAtFullSize / 100.0) * (min(1.0, PS_math.safedivExcept(self.age, daysToFullSize, 0)))
        if self.isApical:
            return
        try:
            numParts = 5
            ...
            turtle.ifExporting_startPlantPart(self.longNameForDXFPartConsideringGenderEtc(kExportPartMeristem), self.longNameForDXFPartConsideringGenderEtc(kExportPartMeristem))
            if numParts > 0:
                for i in range(0, numParts):
                    ...
                    if tdo != None:
                        ...
                        self.draw3DObject(tdo, scale, self.plant.pAxillaryBud.tdoParams.faceColor, self.plant.pAxillaryBud.tdoParams.backfaceColor, kExportPartMeristem)
                        if i == 1:
            ...
            raise
            PS_support.messageForExceptionType(e, "PdMeristem.draw")
    ...
    def decideIfActiveFemale(self):
        ...
        result = (self.isApical == self.plant.pInflor[kGenderFemale].isTerminal)
    ...
    def decideIfActiveMale(self):
        ...
        result = (self.isApical == self.plant.pInflor[kGenderMale].isTerminal)
        return result
    ...
    def willCreateInflorescence(self):
        ...
        try:
            ...
            if self.isApical:
                ...
                inflorProb = PS_math.safedivExcept(numExpected - numAlready, self.plant.numApicalInactiveReproductiveMeristems, 0)
            else:
                ...
                inflorProb = PS_math.safedivExcept(numExpected - numAlready, self.plant.numAxillaryInactiveReproductiveMeristems, 0)
            if (numExpected <= 3):
            ...
            raise
            result = False
            PS_support.messageForExceptionType(e, "PdMeristem.willCreateInflorescence")
        return result
