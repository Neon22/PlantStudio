...
class PdRandom:
    ...
    def setSeedFromSmallint(self, aSeed):
        ...
        self.seed = aSeed % 32768

    def randomSeedFromTime(self):
        seconds = time.time()
        result = int(seconds * 100)
        return result
    ...
    def randomSmallintSeedFromTime(self):
        ...
        result = int(result * self.zeroToOne())
    ...
    def randomNormalBoundedZeroToOne(self, mean):
        #return normal random number based on mean (and std dev of half mean)
        result = max(0.0, (min(1.0, (self.randomNormal(mean)))))
        return result

    def randomNormalPercent(self, mean):
        #return normal random number based on mean (and std dev of half mean) bounded at 0 and 100
        result = max(0, (min(100, int(self.randomNormal(mean / 100.0) * 100.0))))
        return result

    def randomPercent(self):
        k = int(self.seed / 127773)
        self.seed = int((16807 * (self.seed - (k * 127773))) - (k * 2846))
        if (self.seed < 0.0):
            self.seed = self.seed + 2147483647
        result = int(max(0, (min(100, (self.seed * 0.0000000004656612875 * 100.0)))))
        return result
    ...
    def zeroToOne(self):
        ...
        if (self.seed < 0.0):
            self.seed = self.seed + 2147483647
        result = (max(0.0, (min(1.0, (self.seed * 0.0000000004656612875))))) * 1.0
        return result
