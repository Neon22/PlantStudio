...
def readSingleToTab(fileRef):
    ...
    succesful, result = boundForString(temp, kFieldFloat)
    return result
...
def boundForString(boundString, fieldType):
    ...
    if (boundString == "MIN") or (boundString == "FLT_MIN"):
        ...
        if fieldType == kFieldFloat:
            ...
            number = 32767
    elif (boundString == "PI*2") and (fieldType == kFieldFloat):
        piTimesTwo = PS_math.pi * 2.0
        number = piTimesTwo
    elif (boundString == ""):
        if fieldType == kFieldFloat:
            number = 0.0
...
##        errorString = "Problem in method " + methodName + "."
    errorString = errorString + chr(13) + chr(13) + e.message + methodName
    #PS_math.ErrorMessage(errorString)
