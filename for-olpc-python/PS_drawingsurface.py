...
# root class for drawing and export (KfFileExportSurface) to 3D file in PS_3Dexport
class KfDrawingSurface(object):
    def __init__(self):
        ...
        self.foreColor = UnassignedColor
        self.backColor = UnassignedColor
        self.lineColor = UnassignedColor
        self.circleColor = UnassignedColor
        self.triangles = []
        ...
        for i in range(0, kInitialDrawingSurfaceTriangles + 1):
            self.triangles.append(PS_3dsupport.KfTriangle())
    ...
    def plantPartIDForPoint(self, point):
        points = []
        for i in range(2):
            points.append(PS_3dsupport.KfPoint3D())
        ...
        for triangle in self.triangles:
            if triangle.isLine:
                ...
                if PS_3dsupport.pointInTriangle(point, points):
    ...
    def allocateTriangle(self):
        result = PS_3dsupport.KfTriangle()
    ...
    def basicDrawTriangle(self, triangle):
        ...
        pointArray = []
        for i in range(4):
            pointArray.append(SinglePoint()) # not sure what type of point it shoudl be
