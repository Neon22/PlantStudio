...
class ParamsGeneral(object):
    def __init__(self):
        ...
        self.growthSCurve = PS_math.SCurveStructure()
...
class PdPlant(object):
    ...
    def __init__(self):
        ...
        self.randomNumberGenerator = PS_random.PdRandom()
        self.breedingGenerator = PS_random.PdRandom()
        #
        ...
        self.drawPositionIfFixed = SinglePoint()
        self.normalBoundsRect_pixels = Rect()
    ...
    def initialize3DObjects(self):
        ...
        self.pSeedlingLeaf.leafTdoParams.object3D = PS_tdo.KfObject3D()
        self.pLeaf.leafTdoParams.object3D = PS_tdo.KfObject3D()
        self.pLeaf.stipuleTdoParams.object3D = PS_tdo.KfObject3D()
        self.pInflor[kGenderFemale].bractTdoParams.object3D = PS_tdo.KfObject3D()
        self.pInflor[kGenderMale].bractTdoParams.object3D = PS_tdo.KfObject3D()
        for partType in range(0, kHighestFloralPartConstant + 1):
            self.pFlower[kGenderFemale].tdoParams[partType].object3D = PS_tdo.KfObject3D()
            self.pFlower[kGenderMale].tdoParams[partType].object3D = PS_tdo.KfObject3D()
        self.pAxillaryBud.tdoParams.object3D = PS_tdo.KfObject3D()
        self.pFruit.tdoParams.object3D = PS_tdo.KfObject3D()
        self.pRoot.tdoParams.object3D = PS_tdo.KfObject3D()

    def freeAllDrawingPlantParts(self):
        if (self.firstPhytomer != None):
            traverser = PS_travers.PdTraverser().createWithPlant(self)
    ...
    def draw(self):
        PS_travers.cancelDrawing = False
        if (self.turtle == None):
        ...
        self.turtle.setLineColor(clGreen)
        self.turtle.push()
        ...
        self.turtle.drawingSurface.lineColor = clGreen
        if (self.firstPhytomer != None):
            traverser = PS_travers.PdTraverser().createWithPlant(self)
    ...
    def drawOn(self, destCanvas):
        ...
        try:
            ...
            self.turtle.drawingSurface.foreColor = clGreen
            self.turtle.drawingSurface.backColor = clRed
            self.turtle.drawingSurface.lineColor = clBlue
            ...
            try:
                ...
                PS_support.messageForExceptionType(e, "PdPlant.drawOn")
        finally:
...
##                self.recalculateBounds(kDrawNow)
    ...
    def recalculateBounds(self, drawNow, widget):
        ...
        if not intersectResult:
            ...
            umain.MainForm.updateForChangeToPlantBitmaps()
            return
        bytesThisBitmapWillBe = int(PS_support.rWidth(self.boundsRect_pixels()) * PS_support.rHeight(self.boundsRect_pixels()) * umain.MainForm.screenBytesPerPixel())
        ...
        try:
            ...
            self.resizeCacheIfNecessary(self.previewCache, Point(PS_support.rWidth(self.boundsRect_pixels()), PS_support.rHeight(self.boundsRect_pixels())), kInMainWindow)
    ...
    def fakeDrawToGetBounds(self):
        ...
        try:
            ...
            try:
                ...
                PS_support.messageForExceptionType(e, "PdPlant.fakeDrawToGetBounds")
            self.setBoundsRect_pixels(self.getTurtleBoundsRect())
            self.enforceMinimumBoundsRect()
            self.expandBoundsRectForLineWidth()
        finally:
    ...
    def drawIntoCache(self, gc, cache, size, considerDomainScale, immediate):
        ...
        try:
            ...
            if not self.fixedPreviewScale:
                ...
                drawPosition = SinglePoint(cache.Width / 2, cache.Height - 5)
                ...
                drawnWidth = PS_support.rWidth(self.boundsRect_pixels())
                drawnHeight = PS_support.rHeight(self.boundsRect_pixels())
                if (drawnWidth > 0) and (drawnHeight > 0):
                    newScaleX = PS_math.safedivExcept(self.drawingScale_PixelsPerMm * size.X, drawnWidth, 0.1)
                    if considerDomainScale:
                        newScaleX = PS_math.safedivExcept(newScaleX, udomain.domain.plantDrawScale_PixelsPerMm(), newScaleX)
                    else:
                        newScaleX = newScaleX * 0.9
                    newScaleY = PS_math.safedivExcept(self.drawingScale_PixelsPerMm * size.Y, drawnHeight, 0.1)
                    if considerDomainScale:
                        newScaleY = PS_math.safedivExcept(newScaleY, udomain.domain.plantDrawScale_PixelsPerMm(), newScaleY)
                    else:
            ...
            if not self.fixedDrawPosition:
                ...
                try:
                    ...
                    pass
                self.setBoundsRect_pixels(self.getTurtleBoundsRect())
                bestLeft = cache.Width / 2 - PS_support.rWidth(self.boundsRect_pixels()) / 2
                bestTop = cache.Height / 2 - PS_support.rHeight(self.boundsRect_pixels()) / 2
    ...
    def calculateDrawingScaleToLookTheSameWithDomainScale(self):
        self.drawingScale_PixelsPerMm = PS_math.safedivExcept(self.drawingScale_PixelsPerMm, udomain.domain.plantManager.plantDrawScale_PixelsPerMm, 0)
    ...
    def setBasePoint_pixels(self, aPoint_pixels):
        self.basePoint_mm.x = PS_math.safedivExcept(aPoint_pixels.X, udomain.domain.plantDrawScale_PixelsPerMm(), 0) - udomain.domain.plantDrawOffset_mm().x
        self.basePoint_mm.y = PS_math.safedivExcept(aPoint_pixels.Y, udomain.domain.plantDrawScale_PixelsPerMm(), 0) - udomain.domain.plantDrawOffset_mm().y
    ...
    def moveBy(self, delta_pixels):
        ...
        delta_mm.x = PS_math.safedivExcept(delta_pixels.X, udomain.domain.plantDrawScale_PixelsPerMm(), 0)
        delta_mm.y = PS_math.safedivExcept(delta_pixels.Y, udomain.domain.plantDrawScale_PixelsPerMm(), 0)
...
##        return result
    ...
    def saveToGlobal3DOutputFile(self, indexOfPlant, translate, rectWithAllPlants, outputType, aTurtle):
        ...
        if translate:
            relativeBasePoint_mm.x = PS_math.safedivExcept(self.basePoint_pixels().X - rectWithAllPlants.Left, udomain.domain.plantDrawScale_PixelsPerMm(), 0) - udomain.domain.plantDrawOffset_mm().x
            relativeBasePoint_mm.y = PS_math.safedivExcept(self.basePoint_pixels().Y - rectWithAllPlants.Top, udomain.domain.plantDrawScale_PixelsPerMm(), 0) - udomain.domain.plantDrawOffset_mm().y
            if outputType != u3dexport.kPOV:
    ...
    def defaultParameter(self, param, writeDebugMessage):
        ...
        if param.fieldType == kFieldBoolean:
            ...
            if param.indexType == kIndexTypeSCurve:
                ...
                tempSCurve = PS_math.SCurveStructure().fromString(param.defaultValueString())
    ...
    def writeToPlantFile(self, plantFile):
        tempSCurve = PS_math.SCurveStructure()
                ...
                for j in range(0, section.numSectionItems):
                    ...
                    if param.fieldType == kFieldHeader:
                        ...
                        if self.writingToMemo:
                            ...
                            tempTdo.writeToFileStream(plantFile, PS_tdo.kEmbeddedInPlant)
    ...
    def readLineAndTdoFromPlantFile(self, aLine, plantFile):
        ...
        if (not string_in("[", aLine)) or (not string_in("=", aLine)):
            if self.readingFromMemo:
                ...
                secondLine = PS_support.tolerantReadln(plantFile, secondLine)
            self.readingMemoLine += 1
            udebug.DebugPrint("plant <" + self.name + "> line <" + aLine + "> merged with <" + secondLine + ">")
            aLine = aLine + " " + secondLine
        nameAndFieldID = PS_support.stringUpTo(aLine, "=")
        fieldID = PS_support.stringBeyond(nameAndFieldID, "[")
        fieldID = PS_support.stringUpTo(fieldID, "]")
        if string_match("header", fieldID):
            return
        paramValue = PS_support.stringBeyond(aLine, "=")
        ...
        if param.fieldType == kFieldHeader:
            ...
            if param.indexType == kIndexTypeSCurve:
                ...
                tempSCurve = PS_math.SCurveStructure().fromString(paramValue)
            ...
            if self.readingFromMemo:
                ...
                tempTdo.readFromFileStream(plantFile, PS_tdo.kEmbeddedInPlant)
            tempTdo = self.transferField(kSetField, tempTdo, param.fieldNumber, param.fieldType, 0, False, None)
    ...
    def finishLoadingOrDefaulting(self, checkForUnreadParams):
        ...
        #PS_math.calcSCurveCoeffs(self.pGeneral.growthSCurve)
        #PS_math.calcSCurveCoeffs(self.pFruit.sCurveParams)
        #PS_math.calcSCurveCoeffs(self.pLeaf.sCurveParams)
...
##        return objectValue, value
    ...
    def nextDay(self):
        try:
            ...
            fractionToMaturity_frn = PS_math.safedivExcept(self.age, self.pGeneral.ageAtMaturity, 0)
            newTotalBiomass_pctMPB = max(0.0, min(100.0, 100.0 * PS_math.scurve(fractionToMaturity_frn, self.pGeneral.growthSCurve.c1, self.pGeneral.growthSCurve.c2)))
            ...
            if self.floweringHasStarted:
                ...
                fractionReproBiomassToday_frn = max(0, self.pGeneral.fractionReproductiveAllocationAtMaturity_frn * PS_math.safedivExcept(self.age - self.ageAtWhichFloweringStarted, self.pGeneral.ageAtMaturity - self.ageAtWhichFloweringStarted, 0))
            ...
            try:
                if (self.firstPhytomer == None):
                    ...
                    firstMeristem = PS_merist.PdMeristem().newWithPlant(self)
                ...
                if self.firstPhytomer != None:
                    traverser.ageOfYoungestPhytomer = 10000000 #!! very big long
                else:
            ...
            raise
            PS_support.messageForExceptionType(e, "PdPlant.nextDay")
    ...
    def allocateOrRemoveBiomassWithTraverser(self, traverserProxy):
        ...
        if shootAddition_pctMPB > 0.0:
            ...
            self.unallocatedNewVegetativeBiomass_pctMPB = self.allocateOrRemoveParticularBiomass(shootAddition_pctMPB, self.unallocatedNewVegetativeBiomass_pctMPB, kActivityDemandVegetative, kActivityGrowVegetative, traverser)
        if shootReduction_pctMPB > 0.0:
            # remove dead shoot biomass
            self.unremovedDeadVegetativeBiomass_pctMPB = self.allocateOrRemoveParticularBiomass(shootReduction_pctMPB, self.unremovedDeadVegetativeBiomass_pctMPB, kActivityVegetativeBiomassThatCanBeRemoved, kActivityRemoveVegetativeBiomass, traverser)
        if reproAddition_pctMPB > 0.0:
            # allocate new fruit biomass
            self.unallocatedNewReproductiveBiomass_pctMPB = self.allocateOrRemoveParticularBiomass(reproAddition_pctMPB, self.unallocatedNewReproductiveBiomass_pctMPB, kActivityDemandReproductive, kActivityGrowReproductive, traverser)
        if reproReduction_pctMPB > 0.0:
            # remove dead fruit biomass
            self.unremovedDeadReproductiveBiomass_pctMPB = self.allocateOrRemoveParticularBiomass(reproReduction_pctMPB, self.unremovedDeadReproductiveBiomass_pctMPB, kActivityReproductiveBiomassThatCanBeRemoved, kActivityRemoveReproductiveBiomass, traverser)
        ...
        try:
            ...
            if totalDemandOrAvailableBiomass_pctMPB > 0.0:
                if biomassToAddOrRemove_pctMPB > totalDemandOrAvailableBiomass_pctMPB:
                    ...
                    traverser.fractionOfPotentialBiomass = PS_math.safedivExcept(biomassToAddOrRemove_pctMPB, totalDemandOrAvailableBiomass_pctMPB, 0)
            ...
            raise
            PS_support.messageForExceptionType(e, "PdPlant.allocateOrRemoveParticularBiomass")
        return undistributedBiomass_pctMPB
    ...
    def getPlantStatistics(self, statisticsProxy):
        ...
        statistics.liveBiomass_pctMPB[kStatisticsPartTypeUnallocatedNewVegetativeBiomass] = self.unallocatedNewVegetativeBiomass_pctMPB
        statistics.deadBiomass_pctMPB[kStatisticsPartTypeUnremovedDeadVegetativeBiomass] = self.unremovedDeadVegetativeBiomass_pctMPB
        statistics.liveBiomass_pctMPB[kStatisticsPartTypeUnallocatedNewReproductiveBiomass] = self.unallocatedNewReproductiveBiomass_pctMPB
        statistics.deadBiomass_pctMPB[kStatisticsPartTypeUnremovedDeadReproductiveBiomass] = self.unremovedDeadReproductiveBiomass_pctMPB

    def report(self):
        traverser = PS_travers.PdTraverser()
...
def checkVersionNumberInPlantNameLine(aLine):
    ...
    plantName = PS_support.stringBetween("[", "]", aLine)
    versionNumberString = PS_support.stringBetween("<", ">", aLine)
    versionNumberString = PS_support.stringBetween("v", ".", versionNumberString)
...
class PlantLoader(object):
    ...
    def loadPlantsFromFile(self, fileName, inPlantMover, justLoad=0):
        ...
        try:
            ...
            #PS_support.setDecimalSeparator()
            ...
            while aLine != None:
                ...
                if string_startsWith("offset=", aLine):
                    self.plantDrawOffset_mm = PS_support.stringToSinglePoint(PS_support.stringBeyond(aLine, "="))
                elif string_startsWith("scale=", aLine):
                    try:
                        self.plantDrawScale_PixelsPerMm = int(float(PS_support.stringBeyond(aLine, "=")) * 100.0) / 100.0
                    except:
                        self.plantDrawScale_PixelsPerMm = 1.0
                    ...
                    if not justLoad:
                        ...
                        if udomain.domain.options.ignoreWindowSettingsInFile:
                            ...
                            concentratedInFile = PS_support.strToBool(PS_support.stringBeyond(aLine, "="))
                    ...
                    if not justLoad:
                        if not udomain.domain.options.ignoreWindowSettingsInFile:
                            ...
                            self.mainWindowOrientation = int(PS_support.stringBeyond(aLine, "="))
                elif string_startsWith("boxes=", aLine):
                    self.showBoundsRectangle = PS_support.strToBool(PS_support.stringBeyond(aLine, "="))
                    if not justLoad:
                        if not inPlantMover:
                    ...
                    plant = PdPlant()
                    #plantName = PS_support.stringBeyond(aLine, "[")
                    #plantName = PS_support.stringUpTo(plantName, "]")
                    plantName = PS_support.stringBetween("[", "]", aLine)
